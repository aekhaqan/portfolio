  ...
  return (
    <section className="w-full max-w-6xl mx-auto px-4 py-6">
      <div className="relative bg-white dark:bg-primary border border-neutral-200 dark:border-neutral-800 rounded-3xl p-5 shadow-xl">
              ...
              {Array.from({ length: 18 }, (_, index) => {
                ...
                return (
                  <button
                    ...
                    className={`aspect-square rounded-xl font-bold text-3xl bg-neutral-100 dark:bg-neutral-900 text-black dark:text-white hover:bg-neutral-200 dark:hover:bg-neutral-800 transition-all ${highlightedCard === index ? 'scale-95' : 'hover:scale-105'
                      }`}
                  >
                    {num}
                  </button>
        ...
        {/* Game Over Message */}
        {gameState.gameOver && (
          <div className={`mb-3 p-3 rounded-xl text-center ${gameState.currentPosition >= PHONE_NUMBER.length
            ? 'bg-green-50 dark:bg-green-950 border-2 border-green-500'
            : 'bg-red-50 dark:bg-red-950 border-2 border-red-500'
            }`}>
            <h3 className={`text-lg font-bold mb-1 ${gameState.currentPosition >= PHONE_NUMBER.length
